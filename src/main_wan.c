...
uint8_t reword_word(uint8_t num);
...
//          printf ("\n");
		...
		for (word_pos = 0; word_pos < word_cnt; word_pos++){
				if (word_pos % 16 == 0)
							printf ("\n");
				printf ("[0x%02x] ", reword_word(org_buff[word_pos]));
...
uint8_t reword_word(uint8_t num)
{
	...
	{
		tmp_bit = GetBit(num , loop);
		tmp_bit ? SetBit( ret_num, 7- loop): ClearBit( ret_num, 7- loop);
	}
	return ret_num;	
}
